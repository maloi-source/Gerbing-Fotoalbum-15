...
Option Explicit
    Public GdipInitialized As Boolean                                                   'Gerbing 30.09.2015
    Public Const GdiplusVersion As Long = 1                                             'Gerbing 30.09.2015
    Private GdipToken As Long                                                           'Gerbing 30.09.2015
    ' ===============================================================
...
Public Function Execute(ByVal lReturn As Status) As Status
    ...
    Else
        lCurErr = lReturn
'        Call MsgBox(GdiErrorString(lReturn) & " GDI+ Error:" & lReturn, vbOKOnly, "GDI Error") 'Gerbing 06.04.2017 sonst Out Of Memory. GDI+ Error:3
    End If
    Execute = lCurErr
End Function
...
'------------------------------------------------------
' Funktion     : CreateStdPictureFromFile
' Beschreibung : Lädt ein Bilddatei per GDI+
' Übergabewert : FileName = Pfad\Dateiname der Bilddatei
' Rückgabewert : StdPicture Objekt
'------------------------------------------------------
Public Function CreateStdPictureFromFile(ByVal FileName As String) As StdPicture
    Dim retStatus As Status
    Dim lBitmap As Long
    Dim hBitmap As Long
    ...
    retStatus = Execute(GdipLoadImageFromFile(StrPtr(FileName), _
        lBitmap))
    If retStatus = OK Then
        ' Erzeugen der GDI hbitmap von der lbitmap
        retStatus = Execute(GdipCreateHBITMAPFromBitmap(lBitmap, _
            hBitmap, 0))
        If retStatus = Status.OK Then
            ' Erzeugen des StdPicture Objekts von hBitmap
            Set CreateStdPictureFromFile = _
                HandleToPicture(hBitmap, vbPicTypeBitmap)
        End If
        ' Lösche lBitmap
        Call Execute(GdipDisposeImage(lBitmap))
