   ...
   'On Error GoTo Error
    ...
    Select Case False
        Case lDataLen > 0&, AscW(sOperaVer) <> 48   'AscW("0") = 48
            GetFotosExeVersion = "Unable to get fotos.exe version!"
            MsgBox "Unable to get fotos.exe version!"                               'Gerbing 29.04.2019
        Case Else
            GetFotosExeVersion = sOperaVer
    End Select
