...
Option Explicit
    Private Declare Function GetFileVersionInfoW Lib "version.dll" (ByVal lptstrFilename As Long, _
        ByVal dwHandle As Long, ByVal dwLen As Long, ByRef lpData As Any) As Long
    Private Declare Function GetFileVersionInfoSizeW Lib "version.dll" (ByVal lptstrFilename As Long, _
        Optional ByRef lpdwHandle As Long) As Long
...
Public Function FileInfo(Optional ByVal PathWithFilename As String) As FILEPROPERTIE    'Gerbing 18.05.2018
 ' return file-properties of given file  (EXE , DLL , OCX)
    ...
    ' size
    lngBufferlen = GetFileVersionInfoSizeW(StrPtr(PathWithFilename), lngDummy)          'unicode fähig
    If lngBufferlen > 0 Then
       ReDim bytBuffer(lngBufferlen)
       lngRc = GetFileVersionInfoW(StrPtr(PathWithFilename), 0&, lngBufferlen, bytBuffer(0))    'unicode fähig
       If lngRc <> 0 Then
