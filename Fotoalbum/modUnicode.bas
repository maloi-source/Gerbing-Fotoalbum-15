...
Public Function ShowOpenUnicodeOtherVideoPlayer(Fm As Form) As String
    With opFile
        .flags = &H2 Or &H4
        .hInstance = App.hInstance
        .hWndOwner = Fm.hWnd
        .lpstrFilter = StrConv(("exe Files" & Chr(0) & "*.exe" & Chr(0) & Chr(0)), vbUnicode)
        '.lpstrFilter = StrConv(("All Files" & Chr(0) & "*.*" & Chr(0) & Chr(0)), vbUnicode)
        .lpstrTitle = StrConv("location of video player", vbUnicode)
        .lpstrFile = StrConv(String(256, Chr(0)), vbUnicode)
        .nMaxFile = 512
        .lStructSize = Len(opFile)
    End With
    ...
    ShowOpenUnicodeOtherVideoPlayer = opFile.lpstrFile
End Function

Public Function ShowOpenUnicodeExternalVideoPlayer(Fm As Form) As String
    With opFile
        .flags = &H2 Or &H4
        .hInstance = App.hInstance
        .hWndOwner = Fm.hWnd
        .lpstrFilter = StrConv(("exe Files" & Chr(0) & "*.exe" & Chr(0) & Chr(0)), vbUnicode)
        '.lpstrFilter = StrConv(("All Files" & Chr(0) & "*.*" & Chr(0) & Chr(0)), vbUnicode)
        .lpstrTitle = StrConv("Microsoft Media Player", vbUnicode)
        .lpstrFile = StrConv(String(256, Chr(0)), vbUnicode)
        .nMaxFile = 512
        .lStructSize = Len(opFile)
    End With
    ...
    ShowOpenUnicodeExternalVideoPlayer = opFile.lpstrFile
End Function
