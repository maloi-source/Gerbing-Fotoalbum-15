...
Public Function ShowOpenUnicodeEveryThing(Fm As Form) As String
    With opFile
        .flags = &H2 Or &H4
        .hInstance = App.hInstance
        .hWndOwner = Fm.hWnd
        ...
        '.lpstrFilter = StrConv(("All Files" & Chr(0) & "*.*" & Chr(0) & Chr(0)), vbUnicode)
        .lpstrTitle = StrConv("location of everything.exe", vbUnicode)
        .lpstrFile = StrConv(String(256, Chr(0)), vbUnicode)
        .nMaxFile = 512
        .lStructSize = Len(opFile)
    End With
    ...
    ShowOpenUnicodeEveryThing = opFile.lpstrFile
End Function
